...
// ── Konstante ────────────────────────────────────────────────────
const COL_W = 38;     // px po danu
const ROW_H = 52;     // px po apartmanu
const LABEL_W = 130;  // px za label stupac
...
// Razlika u cijelim danima između dva datuma (oba na midnight)
...
export default function BookingTimeline({ bookings, onEditBooking }: Props) {
  ...
  const months = useMemo(() => {
    return [0, 1].map((i) => {
      ...
      return {
        ...
        startDate: ref, // 1. dan tog mjeseca
      };
    });
  }, [today, offset]);

  // Sve vidljive dane (lijevog + desnog mjeseca)
  ...
  const getBar = (b: Booking) => {
    ...
    return {
      ...
      clipped: leftDays < 0, // rezervacija počela u prethodnom mjesecu
    };
  };

  // Rezervacije vidljive za određeni apartman
  ...
  // Indeks dana u allDays koji je "danas"
          ...
          <div className="flex border-b border-gray-100 bg-gray-50">
            ...
            {months.map((m) => (
              <div
                ...
                className={clsx(
                  'text-center py-2 text-xs font-bold uppercase tracking-widest shrink-0',
            ...
            {allDays.map((day, idx) => {
              ...
              return (
                <div
                  ...
                  style={{ width: COL_W }}
                  className={clsx(
                    'flex flex-col items-center justify-center py-1.5 shrink-0 text-xs',
                    'border-r',
            ...
            return (
              ...
              >
                ...
                <div className="relative flex-1 overflow-hidden" style={{ height: ROW_H }}>
                  {/* Pozadinski grid — vertikalne linije */}
                  {allDays.map((day, idx) => {
                    const isToday = day.getTime() === today.getTime();
                    const isWeekend = day.getDay() === 0 || day.getDay() === 6;
                    const isSep = idx === months[0].days - 1;
